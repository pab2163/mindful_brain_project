...
function [list]= make_TP_trial_face(Rate,HoldTime,framesper,trial_length)
...
%make'list'
list=[];
%for faces- this makes sure there are no repeats of betty
for p=1:3
    repeat=0;
    list_temp=Shuffle([ones(trials_per_phase(p)-mnts_per_phase(p),1)*2;ones(mnts_per_phase(p),1)]);
    if length(list_temp)>2
        for t=2:length(list_temp)
            if list_temp(t)==1 & list_temp(t-1)==list_temp(t)
                repeat=repeat+1;
            end;
        end;
    end;
