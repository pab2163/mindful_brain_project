% Updated for scanning by Monica Rosenberg 9/17/10
% Updated to resize images 9/20/10

clear all; clockrandseed;

%%%%%%%%%%%%%%response codes

r1=54; % ',' for response key 1 (cat2)     male\city

r2=55;  % '.' for response key 2 (cat1)     female\mountain

r1_MRI=5; % red button for response key 1 (cat2)     male\city

r2_MRI=10;  % green button for response key 2 (cat1)     female\mountain

scene_cats={'mountain' 'city'}; %city 2nd

face_cats={'female2' 'male'}; 

block_length=30;

y_scale=.735; %columns/width
x_scale=.608;  %rows/height

%%%%%%%%%%%%%%%%%%%%%%%%%%dialogue box

prompt = {'Enter subject name','Rate','Duration','fMRI','Prac','FHR','Prob','Hold','Scram','Task','framesper'};
def={'XXX','.05','360','1','0','0','.90 .90','.4','0','2','13'};
...
block_trials=round(block_length/(Rate*(framesper-1)+HoldTime));
...
[x,y]=size(jpeg_face);
%to resize jpeg_face%%%%%%
jpeg_face=imresize(jpeg_face,[round(x*x_scale),round(y*y_scale)]);
[x,y]=size(jpeg_face);
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Prob=Probs(1);
face_type=randi(100);  if face_type<=100*Prob  face_type=2; else face_type=1; end;
face_r=randi(size(picture_names{face_type+2},1));
scene_type=randi(100); if scene_type<=100*Prob  scene_type=2; else scene_type=1; end;
...
[x,y]=size(jpeg1);
%to resize jpeg1%%%%
jpeg1=imresize(jpeg1,[round(x*x_scale),round(y*y_scale)]);
[x,y]=size(jpeg1);
%%%%%%%%%%%%%%%%%%%%
...
%to resize jpeg2%%%%%%%%%
[x,y]=size(jpeg2);
jpeg2=imresize(jpeg2,[round(x*x_scale),round(y*y_scale)]);
[x,y]=size(jpeg2);
...
%     centertext2(window,['actual duration: ' num2str(duration)],[0 0 0],0, 200);
    ...
    if face_type==1
        ...
        face_type_r2=randi(100); if face_type_r2<=100*Prob  face_type_r2=2; else face_type_r2=1; end;
    end;
    face_r2=randi(size(picture_names{face_type_r2+2},1));
    while face_r==face_r2 & face_type==face_type_r2
        face_r2=randi(size(picture_names{face_type_r2+2},1));
    end;
    scene_type_r2=randi(100); if scene_type_r2<=100*Prob  scene_type_r2=2; else scene_type_r2=1; end;
    ...
    [x,y]=size(jpeg3);
    %to resize jpeg3%%%%
    jpeg3=imresize(jpeg3,[round(x*x_scale),round(y*y_scale)]);
    [x,y]=size(jpeg3);
    %%%%%%%%%%%%%%%%%%%%
...
%     end;
    ...
    %to resize jpeg4%%%%%%%%%
    [x,y]=size(jpeg4);
    jpeg4=imresize(jpeg4,[round(x*x_scale),round(y*y_scale)]);
    [x,y]=size(jpeg4);
...
if fMRI==1
        centertext2(window,['Get Ready'],[0 0 0 ],0,200);
        KeyPressed='';
        
elseif fMRI==0
        centertext2(window,['Press any key to begin block'],[0 0 0],0, 200);
end;
...
cd data
'saving data- wait!'
save(['Data_' num2str(Prac) '_' subName '_' date '_' num2str(C(4)) '_' num2str(C(5)) '_LOC.mat'],'data', 'response','ttt','FHR','Rate','Prob','Task','Scram', 'framesper', 'numberoftrials', 'subName','starttime','endtime','block_length','block_trials');
cd ..
WaitSecs(1);
