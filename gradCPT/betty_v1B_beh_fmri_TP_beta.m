% March 29 2015- first attempt at betty with TPs

%Thought probe gradCPT betty beta version April 2015

%timing is 1200 per trial/face- 800 transition, 400 hold

%can run show_betty to see target or view gradCPT_April_2015_TP_beta/faces/female1/HKF22E.bmp
...
def={'XXX','.05','NA','0','0','1','.9','.4','0','2','16','9'};
...
for TP=1:TP_trials
    %make a supertrial
    [list_temp]= make_TP_trial_face(Rate,HoldTime,framesper,trial_length);
...
%changed to just scene for mix issues for scramble
jpeg_M1=double(jpeg2);
...
%     CenterText2(window,['actual duration: ' num2str(duration)],[0 0 0],0, 200);
    ...
    face_type_r2=trial_list(n); %use trial_list
...
cd data
'saving data- wait!'
save(['Data_' num2str(Prac) '_' subName '_' date '_' num2str(C(4)) '_' num2str(C(5)) '_betty_v1B_beh_fMRI_TP.mat'],'data', 'response','ttt','FHR','Rate','Prob','Task','Scram', 'framesper', 'numberoftrials', 'subName','starttime','endtime','x_size','y_size','TP_Results','trial_list','start_trials','end_trials');
cd ..
WaitSecs(1);
