...
filename = 'data' + os.path.sep + '%s_DMN_preFeedback_%s_Scale%s' %(expInfo['participant'],expInfo['session'],expInfo['Scale_Factor'])
...
text = visual.TextStim(win=win, ori=0, name='text',
    text=u'Noting Practice\n\nIn this run you will see %s circles after 30 seconds of baseline with a cross (+) in the center of the screen.\n\nPlease rest during this baseline.\
\n\nWhen the circles appear, please start performing the Noting Practice. During this run the central white dot will not move. However, please keep performing the Noting Practice throughout the entire experiment. \n\n\
At the end of the run you will see a picture of your overall performance. \n\nThis experiment will last 2 min.\n\nPress any button to start.' %int(roi_number),font=u'Arial',
...
post_feedback_Clock = core.Clock()
post_feedback_text = visual.TextStim(win=win, ori=0, name='post_feedback_text',
    text=u'Your feedback is:',    font=u'Arial',
    pos=[0, 0], height=0.1, wrapWidth=None,
    color=u'white', colorSpace='rgb', opacity=1,
    depth=0.0)

# Initialize components for Routine "post_feedback_show"
post_feedback_showClock = core.Clock()
polygon = visual.Polygon(win=win, name='polygon',
    ...
    opacity=1,depth=0.0, 
interpolate=True)
post_feedback_circle = visual.Polygon(win=win, name='post_feedback_circle',
...
#communicator = MurfiActivationCommunicator('18.93.13.140',
communicator = MurfiActivationCommunicator('192.168.2.5',
                                           15001, 210,
                                           roi_names,exp_tr,murfi_FAKE)
print ("running communicator")
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        for i in range(n_roi):
            ...
            if roi_activities.index(np.nanmax(roi_activities))==i and np.nanmean(roi_activities)!=0:
                ...
                #cursor_position = np.dot(positions[i], activity) UNCOMMENT THIS FOR REAL FEEDBACK
                cursor_position = np.dot(positions[i], 0)
                ...
                #sum activations for post_feedback display
                
                if 'dmn' in roi_names_list[i]:
                    post_feedback.append(activity*(-1))
                    post_feedback1=np.subtract(post_feedback1,activity)
                else:
                    post_feedback.append(activity)
                    post_feedback1=np.add(post_feedback1,activity)
                
                
                
                #print post_feedback
                #print "post feedback mean: ", len(post_feedback),np.mean(activity)
                #print "post feedback median: ", len(post_feedback),np.median(activity)
                print ("post feedback sub/add: ", post_feedback1)
        ...
        if frame ==14:
            with open(filename+'_ROI_activity.csv', 'a') as csvfile:
...
#------Prepare to start Routine "post_feedback_"-------
t = 0
post_feedback_Clock.reset()  # clock 
frameN = -1
routineTimer.add(5.000000)
# update component parameters for each repeat
# keep track of which components have post_feedback_ed
post_feedback_Components = []
post_feedback_Components.append(post_feedback_text)
for thisComponent in post_feedback_Components:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

#-------Start Routine "post_feedback_"-------
continueRoutine = True
while continueRoutine and routineTimer.getTime() > 0:
    # get current time
    t = post_feedback_Clock.getTime()
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    
    # *post_feedback_text* updates
    if t >= 0.0 and post_feedback_text.status == NOT_STARTED:
        # keep track of start time/frame for later
        post_feedback_text.tStart = t  # underestimates by a little under one frame
        post_feedback_text.frameNStart = frameN  # exact frame index
        post_feedback_text.setAutoDraw(True)
    if post_feedback_text.status == STARTED and t >= (0.0 + (5-win.monitorFramePeriod*0.75)): #most of one frame period left
        post_feedback_text.setAutoDraw(False)
    
    # check if all components have post_feedback_ed
    if not continueRoutine:  # a component has requested a forced-end of Routine
        routineTimer.reset()  # if we abort early the non-slip timer needs reset
        break
    continueRoutine = False  # will revert to True if at least one component still running
    for thisComponent in post_feedback_Components:
        if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
            continueRoutine = True
            break  # at least one component has not yet post_feedback_ed
...
#-------Ending Routine "post_feedback_"-------
for thisComponent in post_feedback_Components:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
        

#------Prepare to start Routine "post_feedback_show"-------
 
if post_feedback1 > 5:
    post_feedback1=3
elif post_feedback1 <-5:
    post_feedback1=-3
    
t = 0
post_feedback_showClock.reset()  # clock 
frameN = -1
routineTimer.add(6.000000)
# update component parameters for each repeat
# keep track of which components have finished
post_feedback_showComponents = []
post_feedback_showComponents.append(polygon)
post_feedback_showComponents.append(post_feedback_circle)
post_feedback_showComponents.append(polygon_3)
for thisComponent in post_feedback_showComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED

#-------Start Routine "post_feedback_show"-------
continueRoutine = True
while continueRoutine and routineTimer.getTime() > 0:
    # get current time
    t = post_feedback_showClock.getTime()
    frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
    # update/draw components on each frame
    ...
    if polygon.status == STARTED and t >= (0.0 + (6.0-win.monitorFramePeriod*0.75)): #most of one frame period left
        polygon.setAutoDraw(False)
    
    # *post_feedback_circle* updates
    if t >= 0.0 and post_feedback_circle.status == NOT_STARTED:
        # keep track of start time/frame for later
        post_feedback_circle.tStart = t  # underestimates by a little under one frame
        post_feedback_circle.frameNStart = frameN  # exact frame index
        
        #SETS THE ACCUMULATED FEEDBACK DURING THE RUN to the proportion between the target circles
        
        post_feedback_circle.pos=(0,(post_feedback1*0.3))
        post_feedback_circle.setAutoDraw(True)
    if post_feedback_circle.status == STARTED and t >= (0.0 + (6.0-win.monitorFramePeriod*0.75)): #most of one frame period left
        post_feedback_circle.setAutoDraw(False)
    ...
    if polygon_3.status == STARTED and t >= (0.0 + (6.0-win.monitorFramePeriod*0.75)): #most of one frame period left
...
#-------Ending Routine "post_feedback_show"-------
for thisComponent in post_feedback_showComponents:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
