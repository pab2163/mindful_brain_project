...
# paths
cu_path = '/Volumes/columbia/mbNF_MDD/DATA/MRI/flywheel_dicom/flywheel/auerbach/REMIND/*'
neu_path = '/Volumes/columbia/mbNF_MDD/DATA/MRI/from_neu/sourcedata/*'
...
# Print ids
print(sorted(unique_remind_ids))
print(f'Total of {len(unique_remind_ids)} participants found')

# paths to prepped dicoms/bids data
prepped_dicom_path='/neurodata/mindful_brain_project/data/dicom_preppred_for_heudiconv'
...
for id in unique_remind_ids:
	...
	if os.path.isdir(f'prepped_dicom_path/{id}'):
		print(f'Dicom prep already exists for {id}')
	else:
		print(prep_command)
	...
	if os.path.isdir(f'bids_path/sub-{id}'):
		print(f'BIDSified data already exists for {id}')
	else:
		print(heudiconv_command_cleanup)
		print(heudiconv_command_loc)
		print(heudiconv_command_nf)
