%feedback added
%if you want to see zscores again paste
%'[zscore]=norm_calc2(age,response);' into command window
...
prompt = {'Enter subject name','Rate','Duration','fMRI','Prac','FHR','Prob','Hold','Scram','Task','framesper','age'};
def={'XXX','.05','30','0','0','0','.9','.05','0','2','16','21'};
answer = inputdlg(prompt, 'Experimental setup information',1,def);
[subName, Rate, duration, fMRI, Prac, FHR, Prob, Hold,SR,Task,framesper,age] = deal(answer{:});
...
[zscore]=norm_calc2(age,response);

CenterText2(window,['Error z score= ' num2str(round(zscore(1)*100)/100)],[0 0 0],0, -30);
CenterText2(window,['STD z score= ' num2str(round(zscore(2)*100)/100)],[0 0 0],0, 30);

zzz=normcdf(1-zscore); %zzz will be a score 0-1, 1=better 0=worse
...
key=0; %wait for '7' to move on
while key~=36
[secs, keyCode, deltaSecs] = KbWait; key=find(keyCode);
end;
