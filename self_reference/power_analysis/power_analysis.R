...
for (effect_size in effect_sizes){
  ...
  eff_df_dose$power[eff_df_dose$effect_sizes == effect_size] = sim_treat$x
...
for (effect_size in effect_sizes){
  ...
  eff_df_main$power[eff_df_main$effect_sizes == effect_size] = sim_treat$x
...
p = ggplot(data = eff_df, aes(x = omega2, y = power/10, color = type)) +
  geom_hline(yintercept = 80, lty = 2) +
...
write.csv(eff_df, file = 'Downloads/effectsize_df.csv')
ggsave(p, file = 'Downloads/effectsizeplot.png')
