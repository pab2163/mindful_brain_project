...
round2 = readxl::read_excel('emote_second_wordset_positive_negative.xls') %>%
    dplyr::filter(is.na(researcher_remove)) %>%
    mutate(round = '2') 
...
round2 = dplyr::left_join(round2, dplyr::select(emote, word, arousal =adim2), by= 'word')
...
# try anticlustering
covariates = dplyr::select(round2,valence, log_freq, arousal) %>% mutate_all(scale)
